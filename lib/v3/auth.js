...
const { sessionFromRequest: userSessionFromRequest } = require("./user-auth");
...
function issueSession(id) {
  ...
  const payload = b64url(JSON.stringify({ id, root: true, exp: Date.now() + 8 * 60 * 60 * 1000 }));
...
async function adminIdentity(req) {
  const root = sessionFromRequest(req);
  if (root) return { authenticated: true, root: true, id: root.id, role: "admin" };

  const userSession = userSessionFromRequest(req);
  if (!userSession?.id) return { authenticated: false };
  try {
    const user = await getUser(userSession.id);
    if (user?.status === "active" && user?.role === "admin") {
      return { authenticated: true, root: false, id: user.id, role: "admin", user };
    }
  } catch {}
...
async function isAdminRequest(req) {
  return (await adminIdentity(req)).authenticated;
}

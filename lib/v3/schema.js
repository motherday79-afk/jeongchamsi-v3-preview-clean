...
function heroTone(v, fallback = "default") { const value = String(v || "").trim(); return ["default","white","mint","yellow","dark"].includes(value) ? value : fallback; }
function image(v) {
...
function sanitize(domain, input) {
  ...
  if (domain === "politicianPhotos") {
    ...
    for (const raw of (Array.isArray(input?.items) ? input.items : []).slice(0, 543)) {
      ...
      const sourceType = ["manual","auto-wikimedia","auto-official-review"].includes(String(raw?.sourceType || "")) ? String(raw.sourceType) : "manual";
      items.push({
        id,
  ...
  if (domain === "brand") {
    return {
      hero: {
        ...
        productKicker: text(input?.hero?.productKicker || "JEONGCHAMSI", 60),
        productTagline: text(input?.hero?.productTagline || "정치에 참여할 시간", 60),
        productHeadline: text(input?.hero?.productHeadline, 180),
        productAccentText: text(input?.hero?.productAccentText === undefined ? "움직이는 것" : input.hero.productAccentText, 80),
        productDescription: text(input?.hero?.productDescription || "인물·이슈·여론·제안을 한곳에서 보고, 비교하고, 직접 참여하세요", 260),
        productPrimaryLabel: text(input?.hero?.productPrimaryLabel || "정참시 응원하기", 60),
        productPrimaryHref: text(input?.hero?.productPrimaryHref || "/about", 300),
        productSecondaryLabel: text(input?.hero?.productSecondaryLabel || "정참시 후원하기", 60),
        productSecondaryHref: text(input?.hero?.productSecondaryHref || "https://toon.at/donate/jungchamsi", 300),
        productHeadlineTone: heroTone(input?.hero?.productHeadlineTone, "white"),
        productAccentTone: heroTone(input?.hero?.productAccentTone, "yellow"),
        productDescriptionTone: heroTone(input?.hero?.productDescriptionTone, "mint"),

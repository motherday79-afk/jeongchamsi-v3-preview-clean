const DOMAINS = new Set(["columns", "community", "news", "polls", "academy", "generation", "nationalEvaluation", "itsme", "comments"]);
...
function sanitize(domain, input) {
  ...
  if (domain === "comments") {
    return {
      items: (input?.items || []).slice(0, 2000).map(c => ({
        id: text(c.id, 120), domain: text(c.domain, 30), postId: text(c.postId, 120), author: text(c.author, 40),

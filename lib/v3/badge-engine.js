...
const NEW_BADGE_KEYS = Object.freeze([
  ...
  'signature-influencer','agenda-leader','public-icon','grand-connector','elite-strategist','operator','jeongcham-mayor','michael'
]);
...
function computeBadgeMetrics(userId, activity={}, domains={}, extras={}){
  ...
  return {
    actionTotal, authoredPosts:owned.length, comments:mine.length, likesGiven:likedGiven, likesReceived, viewsReceived, participationCount, pollCount, generationCount, evaluationCount, academyCount,
...
function evaluateBadgeRules(user={}, activity={}, metrics={}){
  ...
  if(number(metrics.referralsRecruited)>=1000) earned.add('michael');
  progress.michael={current:Math.min(number(metrics.referralsRecruited),1000),target:1000,label:'추천 정참시민'};
  if(String(user.role||'')==='partner') earned.add('jungchamsi-partner');

  const mayorRequired=[...VALID_BADGE_KEYS].filter(key=>!['operator','jeongcham-mayor'].includes(key));
  const mayorCurrent=mayorRequired.filter(key=>earned.has(key)).length;
  progress['jeongcham-mayor']={current:mayorCurrent,target:mayorRequired.length,label:'운영자 제외 배지 완성'};
  if(mayorRequired.every(key=>earned.has(key))) earned.add('jeongcham-mayor');
...
async function loadBadgeStatus(user, activity, extras={}){
  ...
  const metrics=computeBadgeMetrics(user?.id,activity,{community,itsme,columns,news,comments},extras);
...
async function isBadgeUnlocked(user, activity, badgeKey){
  ...
  const { getReferralStatus } = require('./users');

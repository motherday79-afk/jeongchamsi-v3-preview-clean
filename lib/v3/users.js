...
function ensureReferralNumbers(source = {}) {
  const users = Object.fromEntries(Object.entries(source || {}).map(([id, user]) => [id, { ...(user || {}) }]));
  const entries = Object.values(users);
  const existing = entries.map(u => positiveInt(u.referralNumber)).filter(Boolean);
  const uniqueExisting = new Set(existing);
  const allMissing = entries.length > 0 && existing.length === 0;
  let changed = false;

  if (allMissing) {
    entries.sort((a,b) => String(a.createdAt || "").localeCompare(String(b.createdAt || "")) || String(a.id || "").localeCompare(String(b.id || "")))
      .forEach((user, index) => { user.referralNumber = index + 1; changed = true; });
    return { users, changed };
  }

  const used = new Set();
  for (const user of entries) {
    const n = positiveInt(user.referralNumber);
    if (n && !used.has(n)) used.add(n);
    else if (n) { user.referralNumber = 0; changed = true; }
  }
  let next = Math.max(0, ...used) + 1;
  entries.filter(u => !positiveInt(u.referralNumber))
    .sort((a,b) => String(a.createdAt || "").localeCompare(String(b.createdAt || "")) || String(a.id || "").localeCompare(String(b.id || "")))
    .forEach(user => {
      while (used.has(next)) next += 1;
      user.referralNumber = next;
      used.add(next);
      next += 1;
      changed = true;
    });
...
function referralCountMap(users = {}) {
  const map = {};
  for (const user of Object.values(users || {})) {
    const n = positiveInt(user.referredByNumber);
...
async function readUsers() {
  ...
  const migrated = ensureReferralNumbers(parsed);
  if (migrated.changed) await command(["SET", USERS_KEY, JSON.stringify(migrated.users)]);
...
async function getUsersByIds(ids = []) {
  const cleanIds = [...new Set((Array.isArray(ids) ? ids : []).map(cleanId).filter(Boolean))];
  if (!cleanIds.length) return [];
  const users = await readUsers();
  return cleanIds.map(id => users[id]).filter(Boolean).map(publicUser);
}
...
async function createUser(input = {}, forcedRole = "member") {
  ...
  const referralInput = String(input.referralNumber ?? "").trim();
  let referredByNumber = 0;
  if (referralInput) {
    if (!/^\d+$/.test(referralInput) || !positiveInt(referralInput)) return { ok:false, error:"INVALID_REFERRAL_NUMBER" };
    referredByNumber = positiveInt(referralInput);
    const referrerExists = Object.values(users).some(u => positiveInt(u.referralNumber) === referredByNumber);
    if (!referrerExists) return { ok:false, error:"REFERRER_NOT_FOUND" };
  }
  const referralNumber = Math.max(0, ...Object.values(users).map(u => positiveInt(u.referralNumber))) + 1;
...
async function authenticateUser(id, password, knownUser = null) {
  const user = knownUser || await getUser(id);
...
async function getReferralStatus(id) {
  ...
  const user = users[clean];
  if (!user) return null;
  const counts = referralCountMap(users);
  const referralNumber = positiveInt(user.referralNumber);
  return {
    referralNumber,
    referredByNumber: positiveInt(user.referredByNumber),
    recruitedCount: counts[referralNumber] || 0,
    michaelTarget: 1000
  };
}

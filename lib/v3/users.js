...
function referralCountMap(users = {}) {
  const map = {};
  for (const user of Object.values(users || {})) { const n = positiveInt(user.referredByNumber); if (n) map[n] = (map[n] || 0) + 1; }
  return map;
}
...
async function getReferralStatus(id) {
  ...
  const user = users[clean];
  if (!user) return null;
  const counts = referralCountMap(users), referralNumber = positiveInt(user.referralNumber);
  return { referralNumber, referredByNumber:positiveInt(user.referredByNumber), recruitedCount:counts[referralNumber] || 0, michaelTarget:1000 };
}

...
function issueSession(id) {
  ...
  const payload = b64url(JSON.stringify({ id: String(id || ""), exp: Date.now() + SESSION_DAYS * 24 * 60 * 60 * 1000 }));
...
function setCookie(res, token, req) {
  ...
  res.setHeader("Set-Cookie", `${COOKIE}=${encodeURIComponent(token)}; Path=/; HttpOnly; SameSite=Lax; Max-Age=${SESSION_MAX_AGE}${secure}`);
}

...
const key = domain => `jcv3:content:v3:${domain}`;

async function getJSON(domain) {
  const raw = await command(["GET", key(domain)]);
  if (!raw) return null;
  try { return JSON.parse(raw); } catch { return null; }
}

async function setJSON(domain, data) {
  return command(["SET", key(domain), JSON.stringify(data)]);
}

async function mgetJSON(domains) {
  const raw = await command(["MGET", ...domains.map(key)]);
...
module.exports = { command, getJSON, setJSON, mgetJSON };

...
import { getUserSession, isPostLiked } from "../core/user.js?v=alpha6.0.36.16-badge-eval-hero";
...
export async function renderBoardWriter(domain, search = "") {
  ...
  const communityFields = domain === "community"
    ...
    : `<label>제목<input name="title" maxlength="120" required value="${esc(old?.title || "")}"></label><label>한 줄 요약<input name="summary" maxlength="240" value="${esc(old?.summary || "")}"></label>${isAdmin && config.image ? `<label>대표사진<input type="file" accept="image/*" data-cover-input></label><div class="image-uploader"><div class="image-preview" data-cover-preview data-cover-data="${esc(cover)}" ${cover ? `style="background-image:url('${cover}')"` : ""}>${cover ? "" : "대표사진 미리보기"}</div><div class="image-help"><b>권장 1200×675px · 16:9</b><span>관리자가 게시판에서 직접 작성할 때도 대표사진을 업로드할 수 있습니다.</span></div></div>` : ""}<label>내용<textarea name="body" rows="14" maxlength="50000" required>${esc(old?.body || "")}</textarea></label>`;

  return pageShell(`<main class="subpage"><section class="page-hero"><span class="eyebrow">WRITE · ${config.eyebrow}</span><h1>${old ? `${config.title} 글 수정` : `${config.title} 글쓰기`}</h1><p>${isAdmin ? "관리자 권한으로 게시판에서 직접 등록·수정합니다." : "제목과 내용만 작성하면 바로 게시됩니다."}</p></section><section class="content-card"><form class="member-post-form" data-user-post-form="${domain}" data-item-id="${esc(old?.id || "")}">${communityFields}<div class="auth-error" data-user-post-error></div><div class="admin-form-actions"><button class="primary-btn" type="submit">${old ? "수정 저장" : "등록"}</button><button class="ghost-btn" type="button" data-go="${old ? `/${config.route}/${esc(old.id)}` : `/${config.route}`}">취소</button></div></form></section></main>`);
}

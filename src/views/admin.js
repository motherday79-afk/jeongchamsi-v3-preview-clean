...
function peoplePanel() {
  const all=listAllPoliticians(),a=all.filter(x=>x.type==="assembly"&&x.connected).length,m=all.filter(x=>x.type==="metropolitan"&&x.connected).length,b=all.filter(x=>x.type==="basic"&&x.connected).length;
  return `<section class="admin-panel"><div class="admin-panel-head"><div><h2>인물 데이터 관리</h2><span class="status-pill"><b>PHASE 1</b>사진 제외</span></div><button class="primary-btn" type="button" data-people-sync-assembly>국회의원 공식데이터 동기화</button></div><div class="people-admin-grid"><article><b>국회의원</b><strong>${a} / 300</strong><span>열린국회정보 연결</span></article><article><b>광역단체장</b><strong>${m} / 16</strong><span>다음 단계</span></article><article><b>기초단체장</b><strong>${b} / 227</strong><span>다음 단계</span></article><article><b>인물 공급자</b><strong>${PERSON_PROVIDER_STATUS}</strong><span>공식 API → Redis</span></article><article><b>사진 공급자</b><strong>${PHOTO_PROVIDER_STATUS}</strong><span>1차 데이터에서 제외</span></article><article><b>NOW ENGINE</b><strong>DEFERRED</strong><span>뉴스·키워드 이후</span></article></div><div class="notice-box">외부 API는 관리자 동기화 때만 호출하고, 사용자 화면은 Redis에 저장된 정참시 데이터를 읽습니다. 사진은 수집·저장하지 않습니다.</div><div class="save-state" data-people-sync-state>Vercel 환경변수 OPEN_ASSEMBLY_API_KEY가 설정되어 있으면 동기화할 수 있습니다.</div></section>`;
}

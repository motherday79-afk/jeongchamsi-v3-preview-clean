...
export async function renderPolls() {
  ...
  return pageShell(`<main class="subpage"><section class="page-hero"><span class="eyebrow">CITIZENS’ CHOICE</span><h1>시민들의 선택</h1><p>진행 중인 설문을 직접 선택하고 결과를 확인하는 정참시 설문게시판입니다.</p></section><section class="content-card">${items.length ? `<div class="poll-page-list">${items.map(poll => { const voted = session.authenticated && hasVotedPoll(poll.id); const total = (poll.options || []).reduce((s, x) => s + Number(x.votes || 0), 0); return `<article class="poll-live-card"><span class="status-pill"><b>POLL</b>${voted ? "참여완료" : "진행중"}</span><h2>${esc(poll.question)}</h2><p>${esc(poll.description || "정참시 참여자 기반 설문")} · ${total.toLocaleString("ko-KR")}명 참여</p><div class="poll-choice-list">${(poll.options || []).map(opt => `<button type="button" ${session.authenticated && !voted ? `data-poll-vote data-poll-id="${esc(poll.id)}" data-option-id="${esc(opt.id)}"` : "disabled"}><span>${esc(opt.label)}</span><i><em style="width:${pct(opt, poll.options)}%"></em></i><b>${pct(opt, poll.options)}%</b></button>`).join("")}</div>${session.authenticated ? (voted ? `<small>이 설문에 이미 참여했습니다.</small>` : `<small>하나의 선택지를 눌러 투표하세요.</small>`) : `<div class="member-login-prompt"><span>투표는 로그인 회원 기준으로 한 번 참여할 수 있습니다.</span><button class="primary-btn" type="button" data-go="/login">로그인</button></div>`}</article>`; }).join("")}</div>` : `<div class="empty-state tall"><h2>등록된 설문이 없습니다.</h2><p>관리자에서 설문을 만들면 이곳과 메인에 동시에 표시됩니다.</p></div>`}</section></main>`);
}

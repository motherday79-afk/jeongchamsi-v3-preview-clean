...
export async function renderPolls() {
  ...
  return pageShell(`<main class="subpage"><section class="page-hero"><span class="eyebrow">CITIZENS’ CHOICE</span><h1>시민들의 선택</h1><p>진행 중인 설문을 직접 선택하고 결과를 확인하는 정참시 설문게시판입니다.</p></section><section class="content-card">${items.length ? `<div class="poll-page-list">${items.map(poll => { const voted = session.authenticated && hasVotedPoll(poll.id); const total = (poll.options || []).reduce((s, x) => s + Number(x.votes || 0), 0); return `<article class="poll-live-card"><span class="status-pill"><b>POLL</b>${voted ? "참여완료" : "진행중"}</span><h2>${esc(poll.question)}</h2><p>${esc(poll.description || "정참시 참여자 기반 설문")} · 총 ${total.toLocaleString("ko-KR")}표</p><div class="poll-choice-list">${(poll.options || []).map(opt => `<button type="button" ${session.authenticated && !voted ? `data-poll-vote data-poll-id="${esc(poll.id)}" data-option-id="${esc(opt.id)}"` : "disabled"}><span>${esc(opt.label)}</span><i><em style="width:${pct(opt, poll.options)}%"></em></i><b>${pct(opt, poll.options)}%</b></button>`).join("")}</div>${session.authenticated ? (voted ? `<small>이 설문에 이미 참여했습니다.</small>` : `<small>하나의 선택지를 눌러 투표하세요.</small>`) : `<div class="member-login-prompt"><span>투표는 로그인 회원 기준으로 한 번 참여할 수 있습니다.</span><button class="primary-btn" type="button" data-go="/login">로그인</button></div>`}</article>`; }).join("")}</div>` : `<div class="empty-state tall"><h2>등록된 설문이 없습니다.</h2><p>관리자에서 설문을 만들면 이곳과 메인에 동시에 표시됩니다.</p></div>`}</section></main>`);
}
...
export async function renderCompare(search = "") {
  ...
  const result = pa && pb ? `<section class="content-card"><div class="compare-demo"><div><span class="fake-avatar a"></span><h2>${esc(slotLabel(pa))}</h2><p>실데이터 연결 전</p></div><div class="compare-demo-bars"><label>기본정보 <i><em style="width:0%"></em></i>—</label><label>경력 <i><em style="width:0%"></em></i>—</label><label>정책·공약 <i><em style="width:0%"></em></i>—</label><label>정참시 데이터 <i><em style="width:0%"></em></i>—</label></div><div><span class="fake-avatar b"></span><h2>${esc(slotLabel(pb))}</h2><p>실데이터 연결 전</p></div></div><div class="notice-box">빈 정보끼리도 비교 화면이 정상 작동합니다. 이후 543개 동일 ID에 실제 데이터가 연결되면 이 결과표에 값만 채워집니다.</div></section>` : "";
  return pageShell(`<main class="subpage"><section class="page-hero"><span class="eyebrow">COMPARE POLITICIANS</span><h1>정치인 비교분석</h1><p>현재는 543개 정치인 슬롯을 직접 선택할 수 있습니다. 실제 정보가 비어 있어도 비교 입력과 결과 레이아웃은 완성되어 있습니다.</p></section><section class="content-card"><form class="compare-picker" data-compare-form><label>정치인 A<select name="a" required><option value="">선택</option>${personOptions(a)}</select></label><span class="compare-vs">VS</span><label>정치인 B<select name="b" required><option value="">선택</option>${personOptions(b)}</select></label><button class="primary-btn" type="submit">비교하기</button></form></section>${result}</main>`);
}
...
export async function renderGeneration() {
  ...
  const cards = ages.map(age => {
    ...
    const top = sorted[0];
    const total = sorted.reduce((s, x) => s + Number(x[1] || 0), 0);
    ...
    return `<article><b>${age}</b><p>${esc(label)}</p><strong>${share}%</strong><span>${total}표</span></article>`;
  }).join("");
  ...
  if (session.authenticated) {
    ...
    else voteArea = `<form class="generation-vote-form generation-vote-fixed" data-generation-vote-form><input type="hidden" name="ageGroup" value="${esc(ageGroup)}"><label>내 세대<input value="${esc(ageGroup)}" disabled></label><label>대통령으로 선택할 정치인<select name="personId" required><option value="">정치인 선택</option>${personOptions("", data.candidates || [])}</select></label><button class="primary-btn" type="submit">투표하기</button><div class="save-state" data-generation-vote-state></div></form>`;
  }

  return pageShell(`<main class="subpage"><section class="page-hero"><span class="eyebrow">GENERATION CHOICE · MOCK VOTE</span><h1>세대가 뽑은 대통령</h1><p>10대부터 60대+까지 정참시 참여자의 모의투표입니다. 실제 선거 결과가 아니며, 현재 543개 정치인 슬롯 중 한 명을 선택해 기능을 검수할 수 있습니다.</p></section><section class="content-card"><div class="generation-page-grid">${cards}</div></section><section class="content-card"><div class="section-title"><h2>모의투표 참여</h2><span>${session.authenticated ? "회원 출생연도 기준 세대 자동 적용" : "로그인 필요"}</span></div>${voteArea}</section></main>`);
}

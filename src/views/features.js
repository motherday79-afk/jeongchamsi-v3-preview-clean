...
function adminCompareConditionReadout(value){
  const n=Number(value);
  return Number.isFinite(n)?`${adminCompareConditionLabel(n)} · ${adminCompareNumber(n)}`:"판독 중";
}
...
function adminComparePersonCard(person,entry,index){
  ...
  return `<article class="admin-compare-person-card"><span class="admin-compare-order">#${index+1}</span>${personPhotoMarkup(person,"admin-compare-avatar",{eager:true,size:120})}<div><h3>${esc(person.name)}</h3><p>${esc([person.party,person.jurisdiction].filter(Boolean).join(" · "))}</p><strong>현재 정치 흐름 ${esc(adminCompareConditionReadout(condition))}</strong><small>${esc(diagnosis)} · 신뢰도 ${Number.isFinite(confidence)?Math.round(confidence):"—"}%</small></div><button class="ghost-btn" type="button" data-go="/person/${esc(person.id)}">상세</button></article>`;
}
...
const ADMIN_COMPARE_SIGNAL_METRICS=[
  ...
  ["대중 확산","EXPANSION",e=>e?.person?.summary?.latest?.scores?.massExpansion],
...
function adminCompareExecutive(people,entries){
  ...
  return `<section class="admin-compare-executive admin-compare-decision-room" data-legacy-contract="EXECUTIVE COMPARISON SUMMARY">
    <div class="admin-compare-verdict-copy"><span>JCS EXECUTIVE VERDICT</span><small>결론부터 보겠습니다.</small><h2>${esc(leadName)}가 현재 비교군에서 가장 많은 핵심 신호를 선점하고 있습니다.</h2><p><b>${esc(leadName)}</b>가 ${ADMIN_COMPARE_SIGNAL_METRICS.length}개 관측축 가운데 <strong>${leadWins}개</strong>에서 선두입니다. ${runner?`다음은 <b>${esc(runnerName)}</b>. `:''}가장 크게 벌어진 축은 <b>${esc(widest?.label||'—')}</b>${wideLeader?`이며 <b>${esc(wideLeader)}</b>가 앞섭니다.`:'.'}</p><div class="admin-compare-verdict-note"><span>현재 정치 흐름 선두</span><b>${esc(conditionLead||'—')}</b><em>지지율이 아니라 최근 관심·지지 기반·미디어·이슈 신호를 종합한 상대 판독입니다.</em></div></div>
...
function adminCompareBattlefield(people,entries){
  ...
  const picks=[['EXPANSION','대중 확산','관심이 기존 관심층 밖으로 퍼지는 정도'],['ENGAGEMENT','심층 관심','단순 노출을 넘어 더 알아보려는 관심의 깊이'],['RESILIENCE','정치 회복력','이슈 충격 뒤 관심과 지지 기반이 회복되는 힘'],['MEDIA','미디어 확산','뉴스·SNS 등에서 메시지가 퍼지는 정도']];
  return `<section class="content-card admin-compare-battlefield"><div class="admin-compare-section-kicker"><span>BATTLEFIELD SNAPSHOT</span><h2>차이가 가장 크게 나는 네 영역</h2><p>복잡한 숫자를 모두 읽지 않아도, 현재 누가 어떤 영역에서 앞서는지 먼저 보여줍니다.</p></div><div class="admin-compare-battlefield-grid">${picks.map(([code,title,label],index)=>{const row=stats.rows.find(x=>x.code===code),leader=adminCompareRowLeader(row,people);return `<article><header><span>0${index+1}</span><small>${esc(code)}</small></header><h3>${esc(title)}</h3><strong>${esc(leader.name)}</strong><p>${esc(label)} · 현재 선두 · ${leader.value===null?'—':adminCompareNumber(leader.value)}</p><footer><span>2위와의 격차</span><b>${adminCompareNumber(leader.spread)}</b></footer></article>`;}).join('')}</div></section>`;
}
function adminCompareAdvantageLanes(people,entries){
  ...
  return `<section class="content-card admin-compare-advantage-lanes"><div class="admin-compare-section-kicker"><span>ADVANTAGE LANES</span><h2>누가 어디에서 앞서고 밀리는가</h2><p>관심·지지 기반·미디어·이슈를 묶어 인물별 강점과 약점을 한눈에 비교합니다.</p></div><div class="admin-compare-lane-list">${lanes.map(([code,label,codes])=>{const counts=people.map((person,index)=>({person,index,wins:0}));stats.rows.filter(row=>codes.includes(row.code)).forEach(row=>{if(row.leaders.length===1)counts[row.leaders[0]].wins+=1;});const sorted=[...counts].sort((a,b)=>b.wins-a.wins||a.index-b.index),max=Math.max(1,codes.length),leader=sorted[0];return `<article><div class="admin-compare-lane-label"><small>${esc(code)}</small><b>${esc(label)}</b><span>${esc(leader?.person?.name||'—')} 우위</span></div><div class="admin-compare-lane-bars">${counts.map(row=>`<p><span>${esc(row.person?.name||'—')}</span><i><em style="width:${Math.max(4,row.wins/max*100)}%"></em></i><strong>${row.wins}/${max}</strong></p>`).join('')}</div></article>`;}).join('')}</div></section>`;
}
...
function adminComparePicker(selected=[],executed=false){
  ...
  return `<section class="content-card compare-picker-card admin-compare-picker-card admin-compare-command-deck">
    ...
    <div class="admin-compare-deck-head"><div><span>COMPARE CONTROL</span><h2>비교할 정치인을 선택하세요</h2><p>2–5명을 선택한 뒤 비교하기를 누르면, 누가 어떤 영역에서 앞서고 밀리는지 관리자 전용 분석을 한 번에 보여줍니다.</p></div><div class="admin-compare-count"><strong>${ids.length}</strong><span>/ 5</span><small>현재 ${ids.length}명 선택 · 최소 2명 · 최대 5명</small></div></div>
...
async function renderAdminCompare(search=""){
  ...
  if(execute&&people.length>=2){
    ...
    result=`<section class="content-card admin-compare-ready-state"><div class="admin-compare-ready-icon">JCS</div><div><span>READY TO COMPARE</span><h2>${people.length}명의 비교 대상이 준비되었습니다</h2><p>선택을 확인한 뒤 <b>비교하기</b>를 누르면 연령·성별 흐름, 과거 변화, 핵심 지표를 한 번에 불러옵니다.</p></div></section>`;
  } else {
  ...
  return pageShell(`<main class="subpage compare-live-page admin-compare-page"><section class="page-hero admin-compare-hero"><span class="eyebrow">ADMIN INTELLIGENCE COMPARE</span><h1>관리자 다자간 정치 인텔리전스 비교</h1><p>2–5명의 정치인을 한 번에 비교해 누가 어떤 영역에서 앞서고 밀리는지, 그리고 현재 정치 흐름이 어떤지 쉽게 확인합니다.</p></section>${adminComparePicker(ids,execute)}${result}</main>`);
}

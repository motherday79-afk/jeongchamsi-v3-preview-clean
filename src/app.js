...
async function resolveView(state) {
  ...
  if (!p.length) return (await import("./views/home.js?v=alpha6.0.36.15-ops-sync")).renderHome();
  if (["login", "join", "mypage"].includes(p[0])) {
    const view = await import("./views/user.js?v=alpha6.0.36.15-ops-sync");
  ...
  if (p[0] === "person") return (await import("./views/people.js?v=alpha6.0.36.15-ops-sync")).renderPersonDetail(p[1] || "");
  if (["column", "community", "news"].includes(p[0])) {
    const view = await import("./views/boards.js?v=alpha6.0.36.15-ops-sync");
  ...
  if (p[0] === "admin") return (await import("./views/admin.js?v=alpha6.0.36.15-ops-sync")).renderAdmin();
  if (p[0] === "about") return (await import("./views/brand.js?v=alpha6.0.36.15-ops-sync")).renderAbout();
  if (p[0] === "support") return (await import("./views/brand.js?v=alpha6.0.36.15-ops-sync")).renderSupport();
  if (["guide","privacy","policy"].includes(p[0])) return (await import("./views/legal.js?v=alpha6.0.36.15-ops-sync")).renderLegal(p[0]);
  const view = await import("./views/features.js?v=alpha6.0.36.15-ops-sync");
  ...
  return (await import("./views/home.js?v=alpha6.0.36.15-ops-sync")).renderHome();
}
...
document.addEventListener("click", async event => {
  ...
  if (generationAdd) {
    ...
    if (rows) {
      const tools = await import("./views/generation-admin.js?v=alpha6.0.36.15-ops-sync");
  ...
  if (del) { if (!confirm("이 항목을 삭제할까요?")) return; const r = await (await import("./views/admin.js?v=alpha6.0.36.15-ops-sync")).deleteAdminItem(del.dataset.adminDelete, del.dataset.id); if (!r.ok) alert(`삭제 실패: ${r.error || "저장소 오류"}`); else { clearDomainCache(); await render(currentRoute(), { resetScroll: false }); } return; }
  ...
  if (member) {
    ...
    const r = await (await import("./views/admin.js?v=alpha6.0.36.15-ops-sync")).updateMemberAccess(id, patch);
...
document.addEventListener("input", async event => {
  ...
  if (cover?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.36.15-ops-sync")).prepareCoverPreview(cover.files[0], cover.closest("form")?.querySelector("[data-cover-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); cover.value = ""; } }
  const profile = event.target.closest("[data-profile-input]");
  if (profile?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.36.15-ops-sync")).prepareProfilePreview(profile.files[0], profile.closest("form")?.querySelector("[data-profile-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); profile.value = ""; } }
});

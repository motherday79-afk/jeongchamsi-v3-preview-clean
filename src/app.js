...
async function resolveView(state) {
  ...
  if (!p.length) return (await import("./views/home.js?v=alpha6.0.22-detail-polish")).renderHome();
  if (["login", "join", "mypage"].includes(p[0])) {
    const view = await import("./views/user.js?v=alpha6.0.22-detail-polish");
  ...
  if (p[0] === "person") return (await import("./views/people.js?v=alpha6.0.22-detail-polish")).renderPersonDetail(p[1] || "");
  if (["column", "community", "news"].includes(p[0])) {
    const view = await import("./views/boards.js?v=alpha6.0.22-detail-polish");
  ...
  if (p[0] === "admin") return (await import("./views/admin.js?v=alpha6.0.22-detail-polish")).renderAdmin();
  if (["guide","privacy","policy"].includes(p[0])) return (await import("./views/legal.js?v=alpha6.0.22-detail-polish")).renderLegal(p[0]);
  const view = await import("./views/features.js?v=alpha6.0.22-detail-polish");
  ...
  return (await import("./views/home.js?v=alpha6.0.22-detail-polish")).renderHome();
}
...
document.addEventListener("click", async event => {
  ...
  if (del) { if (!confirm("이 항목을 삭제할까요?")) return; const r = await (await import("./views/admin.js?v=alpha6.0.22-detail-polish")).deleteAdminItem(del.dataset.adminDelete, del.dataset.id); if (!r.ok) alert(`삭제 실패: ${r.error || "저장소 오류"}`); else { clearDomainCache(); await render(currentRoute(), { resetScroll: false }); } return; }
  const member = event.target.closest("[data-member-access]");
  if (member) { const id = member.dataset.memberAccess; const role = document.querySelector(`[data-member-role="${CSS.escape(id)}"]`)?.value || "member"; const status = document.querySelector(`[data-member-status="${CSS.escape(id)}"]`)?.value || "active"; const r = await (await import("./views/admin.js?v=alpha6.0.22-detail-polish")).updateMemberAccess(id, role, status); const st = document.querySelector("[data-member-save-state]"); if (st) st.textContent = r.ok ? "저장 완료" : `저장 실패 · ${r.error || ""}`; if (r.ok) await initializeUserState(); return; }
});
...
document.addEventListener("input", async event => {
  ...
  if (cover?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.22-detail-polish")).prepareCoverPreview(cover.files[0], cover.closest("form")?.querySelector("[data-cover-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); cover.value = ""; } }
  const profile = event.target.closest("[data-profile-input]");
  if (profile?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.22-detail-polish")).prepareProfilePreview(profile.files[0], profile.closest("form")?.querySelector("[data-profile-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); profile.value = ""; } }
});

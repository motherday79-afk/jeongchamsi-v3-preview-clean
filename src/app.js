...
async function resolveView(state) {
  ...
  if (!p.length) return (await import("./views/home.js?v=alpha6.0.17.1-whitefix")).renderHome();
  ...
  if (p[0] === "person") return (await import("./views/people.js?v=alpha6.0.17.1-whitefix")).renderPersonDetail(p[1] || "");
  ...
  if (p[0] === "admin") return (await import("./views/admin.js?v=alpha6.0.17.1-whitefix")).renderAdmin();
  const view = await import("./views/features.js?v=alpha6.0.17.1-whitefix");
  ...
  return (await import("./views/home.js?v=alpha6.0.17.1-whitefix")).renderHome();
}
...
document.addEventListener("input", async event => {
  ...
  if (cover?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.17.1-whitefix")).prepareCoverPreview(cover.files[0], cover.closest("form")?.querySelector("[data-cover-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); cover.value = ""; } }
  const profile = event.target.closest("[data-profile-input]");
  if (profile?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.17.1-whitefix")).prepareProfilePreview(profile.files[0], profile.closest("form")?.querySelector("[data-profile-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); profile.value = ""; } }
});

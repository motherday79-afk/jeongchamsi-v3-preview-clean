...
async function resolveView(state) {
  ...
  if (p[0] === "admin") return (await import("./views/admin.js?v=history-v1-v2-search-daily")).renderAdmin();
...
document.addEventListener("click", async event => {
  ...
  if (nowRefresh) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).runNowDataRefresh();
    if (!r.ok) {
  ...
  if (nowRetry) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).retryNowDataFailures();
  ...
  if (nowFinalize) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).finalizeNowData();
  ...
  if (nowPublish) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).publishNowData();
  ...
  if (historyCaptureCurrent) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).captureHistoryCurrent();
  ...
  if (historyBackfill) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).runHistoryBackfill();
  ...
  if (politicianPhotoCoverage) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).loadPoliticianPhotoCoverageDiagnostic(politicianPhotoCoverage);
  ...
  if (del) { if (!confirm("이 항목을 삭제할까요?")) return; const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).deleteAdminItem(del.dataset.adminDelete, del.dataset.id); if (!r.ok) alert(`삭제 실패: ${r.error || "저장소 오류"}`); else { clearDomainCache(); await render(currentRoute(), { resetScroll: false }); } return; }
  ...
  if (member) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).updateMemberAccess(id, patch);
...
document.addEventListener("input", async event => {
  ...
  if (brandHeroForm) (await import("./views/admin.js?v=history-v1-v2-search-daily")).syncBrandHeroPreview(brandHeroForm);
  ...
  if (pushImage?.files?.[0]) {
    try {
      ...
      const url = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).preparePushImage(pushImage.files[0], holder);
  ...
  if (cover?.files?.[0]) { try { await (await import("./views/admin.js?v=history-v1-v2-search-daily")).prepareCoverPreview(cover.files[0], cover.closest("form")?.querySelector("[data-cover-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); cover.value = ""; } }
  const profile = event.target.closest("[data-profile-input]");
  if (profile?.files?.[0]) { try { await (await import("./views/admin.js?v=history-v1-v2-search-daily")).prepareProfilePreview(profile.files[0], profile.closest("form")?.querySelector("[data-profile-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); profile.value = ""; } }
});
...
document.addEventListener("change", async event => {
  ...
  if (politicianPhoto?.files?.[0]) {
    try {
      ...
      const tools = await import("./views/admin.js?v=history-v1-v2-search-daily");
...
document.addEventListener("submit", async event => {
  ...
  if (form.matches("[data-badge-celebration-form]")) {
    event.preventDefault();
    const r=await (await import("./views/admin.js?v=history-v1-v2-search-daily")).saveBadgeCelebrationConfig(form);
  ...
  if (form.matches("[data-push-form]")) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).sendPushNotification(form, scope);
  ...
  if (form.matches("[data-politician-photo-form]")) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2-search-daily")).savePoliticianPhotoForm(form);
    if (!r.ok) { if (st) st.textContent = `저장 실패 · ${r.error || ""}`; return; }

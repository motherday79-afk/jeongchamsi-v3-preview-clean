...
async function resolveView(state) {
  ...
  if (p[0] === "person") return (await import("./views/people.js?v=alpha6.0.36.18-livebar-auth-generation")).renderPersonDetail(p[1] || "");
  if (["column", "community", "news"].includes(p[0])) {
    const view = await import("./views/boards.js?v=alpha6.0.36.18-livebar-auth-generation");
  ...
  const view = await import("./views/features.js?v=alpha6.0.36.18-livebar-auth-generation");

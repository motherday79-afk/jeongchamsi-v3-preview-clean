...
async function resolveView(state) {
  ...
  if (p[0] === "person") return (await import("./views/people.js?v=03686-history-v2")).renderPersonDetail(p[1] || "");
  ...
  if (p[0] === "admin") return (await import("./views/admin.js?v=history-v1-v2")).renderAdmin();
...
document.addEventListener("click", async event => {
  ...
  if (nowRefresh) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).runNowDataRefresh();
    if (!r.ok) {
  ...
  if (nowRetry) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).retryNowDataFailures();
  ...
  if (nowFinalize) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).finalizeNowData();
  ...
  if (nowPublish) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).publishNowData();
  ...
  if (historyCaptureCurrent) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).captureHistoryCurrent();
    if (!r.ok) alert(`현재 기준점 보존 실패 · ${r.error || ""}`);
    await render(currentRoute(), { resetScroll:false });
    return;
  }
  const historyPersonLoad = event.target.closest("[data-history-person-load]");
  if (historyPersonLoad) {
    const id = document.querySelector("[data-history-person-select]")?.value || "";
    if (!id) { alert("조회할 정치인을 선택해주세요."); return; }
  ...
  if (historyBackfill) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).runHistoryBackfill();
  ...
  if (politicianPhotoCoverage) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).loadPoliticianPhotoCoverageDiagnostic(politicianPhotoCoverage);
  ...
  if (del) { if (!confirm("이 항목을 삭제할까요?")) return; const r = await (await import("./views/admin.js?v=history-v1-v2")).deleteAdminItem(del.dataset.adminDelete, del.dataset.id); if (!r.ok) alert(`삭제 실패: ${r.error || "저장소 오류"}`); else { clearDomainCache(); await render(currentRoute(), { resetScroll: false }); } return; }
  ...
  if (member) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).updateMemberAccess(id, patch);
...
document.addEventListener("input", async event => {
  ...
  if (brandHeroForm) (await import("./views/admin.js?v=history-v1-v2")).syncBrandHeroPreview(brandHeroForm);
  ...
  if (pushImage?.files?.[0]) {
    try {
      ...
      const url = await (await import("./views/admin.js?v=history-v1-v2")).preparePushImage(pushImage.files[0], holder);
  ...
  if (cover?.files?.[0]) { try { await (await import("./views/admin.js?v=history-v1-v2")).prepareCoverPreview(cover.files[0], cover.closest("form")?.querySelector("[data-cover-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); cover.value = ""; } }
  const profile = event.target.closest("[data-profile-input]");
  if (profile?.files?.[0]) { try { await (await import("./views/admin.js?v=history-v1-v2")).prepareProfilePreview(profile.files[0], profile.closest("form")?.querySelector("[data-profile-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); profile.value = ""; } }
});
...
document.addEventListener("change", async event => {
  ...
  if (politicianPhoto?.files?.[0]) {
    try {
      ...
      const tools = await import("./views/admin.js?v=history-v1-v2");
...
document.addEventListener("submit", async event => {
  ...
  if (form.matches("[data-first-admin-setup]")) { event.preventDefault(); const r = await (await import("./views/admin.js?v=history-v1-v2")).submitFirstAdmin(form); const e = form.querySelector("[data-admin-setup-error]"); if (!r.ok) { if (e) e.textContent = r.error || "관리자 생성 실패"; return; } route("/admin", { replace: true }); await render(currentRoute()); return; }
  ...
  if (form.matches("[data-badge-celebration-form]")) {
    event.preventDefault();
    const r=await (await import("./views/admin.js?v=history-v1-v2")).saveBadgeCelebrationConfig(form);
  ...
  if (form.matches("[data-push-form]")) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).sendPushNotification(form, scope);
  ...
  if (form.matches("[data-politician-photo-form]")) {
    ...
    const r = await (await import("./views/admin.js?v=history-v1-v2")).savePoliticianPhotoForm(form);
    if (!r.ok) { if (st) st.textContent = `저장 실패 · ${r.error || ""}`; return; }

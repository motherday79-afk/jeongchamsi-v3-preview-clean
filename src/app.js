...
import {
  ...
  togglePostLike, applyAcademy, getUserSession, refreshUserActivity, updateMyProfile, setRepresentativeBadge, toggleShowcaseBadge
} from "./core/user.js";
...
async function resolveView(state) {
  ...
  if (!p.length) return (await import("./views/home.js?v=alpha6.0.36.24-showcase-hero-now")).renderHome();
  ...
  if (p[0] === "person") return (await import("./views/people.js?v=alpha6.0.36.24-showcase-hero-now")).renderPersonDetail(p[1] || "");
  if (["column", "community", "news"].includes(p[0])) {
    const view = await import("./views/boards.js?v=alpha6.0.36.24-showcase-hero-now");
  ...
  if (p[0] === "admin") return (await import("./views/admin.js?v=alpha6.0.36.24-showcase-hero-now")).renderAdmin();
  if (p[0] === "request-politician") return (await import("./views/participation.js?v=alpha6.0.36.24-showcase-hero-now")).renderPoliticianRequest();
  if (p[0] === "partners") return (await import("./views/participation.js?v=alpha6.0.36.24-showcase-hero-now")).renderPartners();
  if (p[0] === "about") return (await import("./views/brand.js?v=alpha6.0.36.24-showcase-hero-now")).renderAbout();
  if (p[0] === "support") return (await import("./views/brand.js?v=alpha6.0.36.24-showcase-hero-now")).renderSupport();
  if (["guide","privacy","policy"].includes(p[0])) return (await import("./views/legal.js?v=alpha6.0.36.24-showcase-hero-now")).renderLegal(p[0]);
  const view = await import("./views/features.js?v=alpha6.0.36.24-showcase-hero-now");
  ...
  return (await import("./views/home.js?v=alpha6.0.36.24-showcase-hero-now")).renderHome();
}
...
async function hydrateLiveCommunityBar() {
  ...
  let data = null;
  try {
    const response = await fetch(`/api/v3/home?livebar=${Date.now()}`, { credentials:"same-origin", cache:"no-store" });
    const body = await response.json().catch(() => ({}));
    if (response.ok && body?.data) data = body.data;
  } catch {}
  if (data) {
    const config = { useActualCount:true, overrideCount:0, ...(data.brand?.liveBar || {}) };
...
document.addEventListener("click", async event => {
  ...
  if (generationAdd) {
    ...
    if (rows) {
      const tools = await import("./views/generation-admin.js?v=alpha6.0.36.24-showcase-hero-now");
  ...
  if (nationalLive) {
    ...
    const tools = await import("./views/national-evaluation-admin.js?v=alpha6.0.36.24-showcase-hero-now");
  ...
  if (nowMore) {
    ...
    const tools = await import("./views/features.js?v=alpha6.0.36.24-showcase-hero-now");
  ...
  if (del) { if (!confirm("이 항목을 삭제할까요?")) return; const r = await (await import("./views/admin.js?v=alpha6.0.36.24-showcase-hero-now")).deleteAdminItem(del.dataset.adminDelete, del.dataset.id); if (!r.ok) alert(`삭제 실패: ${r.error || "저장소 오류"}`); else { clearDomainCache(); await render(currentRoute(), { resetScroll: false }); } return; }
  ...
  if (member) {
    ...
    const r = await (await import("./views/admin.js?v=alpha6.0.36.24-showcase-hero-now")).updateMemberAccess(id, patch);
...
document.addEventListener("input", async event => {
  ...
  if (cover?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.36.24-showcase-hero-now")).prepareCoverPreview(cover.files[0], cover.closest("form")?.querySelector("[data-cover-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); cover.value = ""; } }
  const profile = event.target.closest("[data-profile-input]");
  if (profile?.files?.[0]) { try { await (await import("./views/admin.js?v=alpha6.0.36.24-showcase-hero-now")).prepareProfilePreview(profile.files[0], profile.closest("form")?.querySelector("[data-profile-preview]")); } catch (e) { alert(e.message || "이미지 처리 실패"); profile.value = ""; } }
});

...
module.exports=async function politicianLiveDataRoute(req,res){
  ...
  if(String(req.query?.status||'')==='1'){
    ...
    return res.status(200).json({ok:true,rosterTotal:allPeople().length,configured:{searchAds:searchCredentials().configured,news:newsCredentials().configured},requiredEnv:{searchAds:['NAVER_AD_ACCESS_LICENSE','NAVER_AD_SECRET_KEY','NAVER_AD_CUSTOMER_ID'],news:['NAVER_API_HUB_CLIENT_ID','NAVER_API_HUB_CLIENT_SECRET']}});
  }

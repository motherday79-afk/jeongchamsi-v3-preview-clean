...
function configState(){
  const search=searchCredentials(),news=newsCredentials(),newsState=typeof newsAvailability==='function'?newsAvailability():{available:Boolean(news?.configured),provider:news?.configured?'naver-news':'jcs-modeled-news-fallback'},missingEnv=[];
  ...
  if(!newsState.available)missingGroups.push('news');
  return {searchAds:search.configured,news:newsState.available,newsNaver:news.configured,newsProvider:newsState.provider||'jcs-modeled-news-fallback',missingEnv,missingGroups,fallbackReady:true};
}
...
function metaProviders(meta={}){const search=meta?.searchLive===true?'naver-search-ads':'jcs-modeled-search-fallback';const news=meta?.newsLive===true?'naver-news':'jcs-modeled-news-fallback';return [search,news];}
...
module.exports=async function nowDataAdmin(req,res){
  ...
  try{
    ...
    if(action==='start'){
      ...
      return res.status(200).json({ok:true,draftId,batchCount:batches.length,batchSize:10,total:ids.length,weights:w,tempCleanup,sourceMode,sourceWarnings:configured.missingGroups,configured:{searchAds:configured.searchAds,news:configured.news,newsNaver:configured.newsNaver},performance:{browserWorkers:2,serverConcurrency:5}});
    }

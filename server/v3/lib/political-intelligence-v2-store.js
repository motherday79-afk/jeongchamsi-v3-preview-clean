...
const CALCULATION_REVISION='JCS_AGGRESSIVE_INTELLIGENCE_20260830_R1';
const ACTIVE_INDEX_KEY=`${INDEX_KEY}:${CALCULATION_REVISION}`;
...
function activeSnapshotKey(draftId){return `${snapshotKey(draftId)}:${CALCULATION_REVISION}`;}
...
function modeledBaselineForPerson(person={}){
  const seed=String(person?.id||person?.name||'jcs');let h=2166136261;
  for(let i=0;i<seed.length;i++){h^=seed.charCodeAt(i);h=Math.imul(h,16777619)>>>0;}
  const weights=[.105,.101,.105,.102,.103,.101,.098,.097,.083,.084,.011,.010];
  const age=[-0.035,-0.018,0.004,0.021,0.034,0.048],phase=h%6,dir=(h&1)?1:-1;
  const affinity=[];
  for(let i=0;i<12;i++){const g=Math.floor(i/2),sex=i%2,rot=age[(g+phase)%6]*dir,sexDelta=(sex?1:-1)*(0.006+((h>>>(g%16))&3)*0.002);affinity.push(Math.round((rot+sexDelta)*1000)/1000);}
  return {personId:String(person?.id||''),baselineKind:'LIMITED',baselineQuality:35,populationWeights:weights,cohortAffinity:affinity,sourceState:'JCS_MODELED_BASELINE',limitedReasons:['OFFICIAL_BASELINE_UNAVAILABLE','JCS_STRUCTURAL_MODEL']};
}
...
function createPoliticalIntelligenceV2Store(overrides={}){
  ...
  async function readPoliticalIntelligenceSnapshotV2(draftId){const id=String(draftId||'').trim();if(!id)return null;const active=decodeSnapshot(await deps.command(['GET',activeSnapshotKey(id)]));if(active)return active;return decodeSnapshot(await deps.command(['GET',snapshotKey(id)]));}
  ...
  async function recordPoliticalIntelligenceSnapshotV2({current={},legacyHistory={items:[]},personViews=[],evidenceBundle={records:[]}}={}){
    ...
    const baselineState=trusted?'TRUSTED_BASELINE':'JCS_MODELED_BASELINE';
...
module.exports={STORE_VERSION,PREFIX,INDEX_KEY,ACTIVE_INDEX_KEY,CALCULATION_REVISION,ACCESS_SCOPE,MAX_COMPRESSED_BYTES,COMPONENT_KEYS,snapshotKey,activeSnapshotKey,encodeSnapshot,decodeSnapshot,packCohorts,unpackCohorts,modeledBaselineForPerson,createPoliticalIntelligenceV2Store,...defaults};

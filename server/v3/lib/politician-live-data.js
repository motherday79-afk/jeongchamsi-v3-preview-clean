...
async function getLiveDataById(id,opts={}){
  ...
  const news=newsResult.status==='fulfilled'?newsResult.value:missing('naver-news',newsResult.reason);
  return {ok:true,person:{id:person.id,type:person.type,name:person.name,party:person.party,jurisdiction:person.jurisdiction,office:person.office,ambiguousName:person.ambiguousName},configured:{searchAds:searchCredentials().configured,news:newsCredentials().configured},providers:['naver-search-ads','naver-news'],search,news,fetchedAt:new Date().toISOString()};
}

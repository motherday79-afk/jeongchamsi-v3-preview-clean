...
async function getLiveDataById(id,opts={}){
  ...
  const rawNews=newsResult.status==='fulfilled'?newsResult.value:missing('naver-news',newsResult.reason);
  const fallback=applyFallbacks(person,rawSearch,rawNews,opts.previousView||null,opts.nowMs||Date.now());
  return {ok:true,person:{id:person.id,type:person.type,name:person.name,party:person.party,jurisdiction:person.jurisdiction,office:person.office,ambiguousName:person.ambiguousName},configured:{searchAds:searchCredentials().configured,news:newsCredentials().configured},providers:[fallback.search?.provider||'search',fallback.news?.provider||'news'],search:fallback.search,news:fallback.news,fetchedAt:new Date().toISOString()};
}

...
const ENGINE_VERSION='JCS_AGE_GENDER_INTELLIGENCE_V2';
...
function cohortProfileEffect(baseline={},cellIndex=0){
  ...
  const weights=Array.isArray(baseline?.populationWeights)&&baseline.populationWeights.length===12?baseline.populationWeights.map(v=>Math.max(0,num(v)||0)):Array(12).fill(1);let sw=0,sv=0;for(let i=0;i<12;i++){if(values[i]===null)continue;const w=weights[i]||0;sw+=w;sv+=w*values[i];}if(!(sw>0))return 0;
  const centered=values[cellIndex]-sv/sw,kind=String(baseline?.baselineKind||''),kindScale=kind==='DIRECT_CANDIDATE'?1:kind==='PARTY_PROXY'?.72:kind==='REGIONAL_PARTY_PROXY'?.58:.5,qualityScale=clamp((Number(baseline?.baselineQuality)||0)/60,.35,1);
  return clamp(centered*30*kindScale*qualityScale,-4,4);
}
...
function computeCell({key,index,baseline,view,history,evidence,events,previous,marketContext={},asOf=null}){
  ...
  if(!baselineUsable(baseline))return {key,value:null,status:'LIMITED_SIGNAL',confidence:null,baselineQuality:Number(baseline?.baselineQuality)||0,evidenceCount:evidenceRows(evidence).length,independentEventCount:events.length,historyDays:hDays,lastValidatedAt:null,reason:'BASELINE_LIMITED',components:{personalEffect:round(global*.5,1),partyEffect:round(party*.1,1),regionalEffect:round(regional*.08,1),competitorEffect:round(competitor*.1,1),issueEffect:round(issue.effect*.23,1),externalAnchorEffect:0,historyPriorEffect:0}};
  const anchorDiag=anchorDiagnostics(evidence,key,index,aff,asOf),external=anchorEffect(detailed!==null?detailed:coarse,aff)*anchorDiag.freshness,prior=previousCell(previous,key),profile=cohortProfileEffect(baseline,index),cap=movementCap({events,evidence,cellKey:key,cellIndex:index,baselineAffinity:aff,asOf});let target,components;
  if(prior){components={personalEffect:global*.14+profile*.8,partyEffect:party*.06,regionalEffect:regional*.05,competitorEffect:competitor*.07,issueEffect:issue.effect*.07,externalAnchorEffect:external*.12,historyPriorEffect:prior.value*.72};target=Object.values(components).reduce((a,b)=>a+b,0);}
  else{const shared=fitSharedComponents({personalEffect:global*.5,partyEffect:party*.1,regionalEffect:regional*.08,competitorEffect:competitor*.1},cap*.65);components={personalEffect:shared.personalEffect+profile,partyEffect:shared.partyEffect,regionalEffect:shared.regionalEffect,competitorEffect:shared.competitorEffect,issueEffect:issue.effect*.23,externalAnchorEffect:external*.38,historyPriorEffect:0};target=Object.values(components).reduce((a,b)=>a+b,0);}
  let value=prior?clamp(target,prior.value-cap,prior.value+cap):clamp(target,-cap,cap);
  value=round(clamp(value,-50,50));const confidence=cellConfidence({baseline,view,history,events,evidence,cellIndex:index,detailedAnchor:detailed,anchorAgreement:anchorDiag.agreement,anchorFreshness:anchorDiag.freshness});const status=confidence>=CONFIDENCE_THRESHOLD?'VALID_SIGNAL':'LIMITED_SIGNAL';
  return {key,value:status==='VALID_SIGNAL'?value:null,status,confidence:status==='VALID_SIGNAL'?confidence:null,baselineQuality:Number(baseline.baselineQuality)||0,evidenceCount:evidenceRows(evidence).length,independentEventCount:events.length,historyDays:hDays,lastValidatedAt:detailed!==null?String(evidenceRows(evidence).find(r=>num(r?.values?.[key])!==null)?.observedAt||'')||null:null,reason:status==='VALID_SIGNAL'?null:'CONFIDENCE_BELOW_THRESHOLD',components:Object.fromEntries(Object.entries(components).map(([k,v])=>[k,round(v,1)]))};
}

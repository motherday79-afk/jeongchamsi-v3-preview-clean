...
function computeCell({key,index,baseline,view,history,evidence,events,previous,marketContext={},asOf=null}){
  ...
  const anchorDiag=anchorDiagnostics(evidence,key,index,aff,asOf),external=anchorEffect(detailed!==null?detailed:coarse,aff)*anchorDiag.freshness,prior=previousCell(previous,key);let target,components;
  if(prior){components={personalEffect:global*.14,partyEffect:party*.06,regionalEffect:regional*.05,competitorEffect:competitor*.07,issueEffect:issue.effect*.07,externalAnchorEffect:external*.12,historyPriorEffect:prior.value*.72};target=Object.values(components).reduce((a,b)=>a+b,0)+(aff||0)*1.5;}
  else{components={personalEffect:global*.5,partyEffect:party*.1,regionalEffect:regional*.08,competitorEffect:competitor*.1,issueEffect:issue.effect*.23,externalAnchorEffect:external*.38,historyPriorEffect:0};target=Object.values(components).reduce((a,b)=>a+b,0)+(aff||0)*3;}
  const cap=movementCap({events,evidence,cellKey:key,cellIndex:index,baselineAffinity:aff,asOf});let value=prior?clamp(target,prior.value-cap,prior.value+cap):clamp(target,-cap,cap);

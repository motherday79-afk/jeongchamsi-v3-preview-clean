...
function summarize(items,person,nowMs,total,query){
  ...
  return {provider:'naver-news-search-api',configured:true,state:recent7.length?'OBSERVED':'ZERO',query,totalCount:Number(total||0),count6:recent6.length,count24:recent24.length,count7d:recent7.length,sources6:new Set(recent6.map(x=>x.source).filter(Boolean)).size,sources24:new Set(recent24.map(x=>x.source).filter(Boolean)).size,latest,headlines:recent7.slice(0,12).map(x=>({title:x.title,source:x.source,ts:x.ts,link:x.link})),fetchedAt:new Date().toISOString()};
}
async function collectNaverNews(person,{nowMs=Date.now()}={}){
  const c=credentials();
  if(!c.configured)return {provider:'naver-news-search-api',configured:false,state:'MISSING',detail:'NAVER News Search API credentials not configured',count6:0,count24:0,count7d:0,sources6:0,sources24:0,headlines:[]};
  ...
  try{r=await fetch(url,{headers:{'X-Naver-Client-Id':c.id,'X-Naver-Client-Secret':c.secret},signal:ctl.signal});}finally{clearTimeout(timer);}
...
module.exports={NEWS_URL,credentials,cleanHtml,buildNewsQuery,mentionsPerson,parseItems,summarize,collectNaverNews};

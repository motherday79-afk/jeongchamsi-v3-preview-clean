...
test('a frozen V2 layer with only LIMITED cohort signal does not replace the useful V1.2 admin fallback',async()=>{
  const {createHistoryV2Store}=require('../server/v3/lib/history-v2-store');
  const v1={version:'JCS_POLITICAL_INTELLIGENCE_V1_2',diagnosis:{label:'V1 USEFUL'}};
  const v2={version:'JCS_POLITICAL_INTELLIGENCE_V2',cohorts:{validity:{state:'LIMITED_SIGNAL',validCellCount:0},cells:{},age:{},gender:{}}};
  const store=createHistoryV2Store({readLatestPoliticalIntelligenceSnapshotPersonV2:async()=>v2,readLatestPoliticalIntelligenceSnapshotPersonV1:async()=>v1,getJSON:async()=>{throw new Error('must not read current');}});
  const out=await store.readPoliticalIntelligenceV2('p');
  assert.equal(out.version,'JCS_POLITICAL_INTELLIGENCE_V1_2');

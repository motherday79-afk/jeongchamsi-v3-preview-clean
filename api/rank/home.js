  ...
  try {
    ...
    if (!response.ok || body?.error) {
      ...
      if (
        ...
        e.code = `REDIS_HTTP_${status || "ERROR"}`;
      }
...
function compactHomeMember(m = {}) {
  return {
    ...
    constituency: String(
      m.constituency || m.jurisdiction || ""
    ),
    overallRank: finite(m.overallRank)
      ? Number(m.overallRank)
      : null,
    rank: finite(m.rank)
      ? Number(m.rank)
      : null,
    categoryRank: finite(m.categoryRank)
      ? Number(m.categoryRank)
      : null,
    previousOverallRank: finite(m.previousOverallRank)
      ? Number(m.previousOverallRank)
      : null,
    previousRank: finite(m.previousRank)
      ? Number(m.previousRank)
      : null,
    changeOverallRefresh: finite(m.changeOverallRefresh)
      ? Number(m.changeOverallRefresh)
      : null,
    changeRefresh: finite(m.changeRefresh)
